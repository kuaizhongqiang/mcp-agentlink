...
export class TokenStore {
   ...
   */
  generate(projectId: string, role: string, tokenId?: string): TokenGenerationResult {
    const raw = randomBytes(32).toString("hex");
    const hash = createHash("sha256").update(raw).digest("hex");

    const tokens = this.db.exec<Token>(
      `INSERT INTO tokens (id, project_id, role, token_hash)
       VALUES (?, ?, ?, ?)
       RETURNING *`,
      [tokenId ?? crypto.randomUUID(), projectId, role, hash]
    );
